...
Support Matrix (Relative)
~~~~~~~~~~~~~~~~~~~~~~~~~
All targets pass through the same high-level pipeline (simplify -> SSA -> RA -> machinize),
but the **target-dependent hooks** vary in maturity and tuning. The matrix below is a
relative snapshot to clarify where each backend is less optimal or still growing.

Legend: optimized (most tuned), solid, growing, baseline.

.. code-block:: none

   Pipeline step                         x86_64    AArch64  RISC-V   PPC64   s390x
   Instruction selection / machinize     optimized solid    growing  baseline baseline
   Addressing & immediate materialization optimized solid   growing  baseline baseline
   Prologue/epilogue & ABI glue           optimized solid    growing  baseline baseline
   Varargs lowering                       optimized solid    growing  baseline baseline
   Peephole / combine patterns            optimized solid    growing  baseline baseline

If you need exact per-opcode gaps, inspect the target backends directly:
`mir/mir-gen-x86_64.c`, `mir/mir-gen-aarch64.c`, `mir/mir-gen-riscv64.c`,
`mir/mir-gen-ppc64.c`, `mir/mir-gen-s390x.c`.

Why RISC-V is marked "growing"
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Concrete gaps are documented in the backend itself. For example, the RISC-V
generator carries an explicit TODO list:
...
These items explain the "growing" label:
- **Compressed branches & relaxation**: missing size optimization and branch-range tuning.
- **Immediate materialization**: missing addi-negation transform leaves extra insns.
- **FP128 (FLEN=128)**: feature coverage not implemented.
- **Prologue/epilogue**: always saving/restoring sp/fp is less optimal.
- **Machinize cleanup**: extra ext32s are left in some branch paths.

Source: `mir/mir-gen-riscv64.c`.
