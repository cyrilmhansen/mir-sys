...
69. The Black Magic: ``_MIR_dump_code``
...
Fear not, brave traveler! Do not let the jagged edges of ``popen`` and system calls frighten you. What looks like a monster is actually your most loyal beast of burden.
...
We have reached the edge of the known world, where the high-level logic of MIR meets the messy, chaotic reality of the Operating System and the CPU. This is not the clean mathematics of the optimizer; this is the grit of the real world.
...
This function performs a dark ritual known as **"Invoking the Shell."**
...
*   **The Problem:** You have a buffer of raw binary bytes (machine code) generated by the JIT. You want to know what it *does*. You could stare at hex codes like ``48 89 E5`` until your eyes bleed, or...



*   **The Hack:** MIR cheats. It doesn't write its own disassembler (which is hard and heavy). Instead, it asks the Operating System for help.



    1.  **The Artifact:** It writes the raw bytes to a temporary file (``_mir_PID.bin`` or ``.c``).



    2.  **The Summoning:** It uses ``popen`` to spawn a child process—specifically ``objdump`` (the standard system disassembler).



    3.  **The Translation:** ``objdump`` reads the binary file, translates it into human-readable Assembly (``mov %rsp, %rbp``), and pipes the text back to MIR.



    4.  **The Revelation:** MIR prints that text to your console.
...
This function is a "Spyglass." It allows you to see the invisible. It is messy, platform-dependent code, but it is a lifesaver when you are debugging why your JIT compiler is crashing.
...
70. The Metamorphosis: Target Inclusion
...
Here, the code undergoes a shapeshifting ritual.
...
This block determines the **Identity** of the compiler.
...
*   MIR is a chameleon. Depending on the flag passed to the C compiler (``-D__x86_64__``, etc.), it pulls in a completely different set of "laws of physics."



*   **The Files:** ``mir-x86_64.c`` contains the definitions of registers like ``RAX`` and instructions like ``MOV``. ``mir-aarch64.c`` contains ``x0`` and ``LDR``.



*   **The Unity:** By including the specific ``.c`` file here, the rest of ``mir.c`` can treat the target abstractly. It doesn't care *which* file was included, only that *one* of them was.
...
If you see the error ``#error "undefined or unsupported generation target"``, it means you have sailed off the edge of the map into unknown hardware territory.
...
71. The True Names: Hard Register Context
...
The final piece of infrastructure is the **Registry of True Names**.
...
*   **The Context:** ``struct hard_reg_ctx``.



*   **The Initialization:** ``hard_reg_name_init``.



    *   This function takes the list of register names defined in the target-specific file included above (e.g., "rax", "rbx", "sp").



    *   It hashes them and stores them in a lookup table.



*   **The Purpose:** This allows the MIR parser to understand when you write ``mov %rax, %rbx``. It looks up "rax", finds it maps to Hard Register #0, and generates the correct internal code.
...
72. The Final Twist: The Interpreter
...
And then, at the very bottom, a plot twist:
...
Just when you thought you were done, the **Interpreter** joins the party. It is not a separate library; it is compiled directly into the heart of MIR. This ensures that even if you cannot generate machine code (perhaps because of security restrictions like W^X), you can always run MIR code slowly and safely via interpretation.
...
Epilogue: The Journey Concludes
...
You have done it. You have traversed the entirety of **``mir.c``**.
...
*   You witnessed the **Creation** of the Context.



*   You saw the **Structures** of Modules and Functions.



*   You navigated the **Binary Serialization** rivers.



*   You survived the **Simplification** passes.



*   And finally, you stood on the precipice of **Machine Code Generation**.
...
The Core is understood. The map is complete.



But the adventure of *using* it—of building languages, optimizing hot paths, and defeating the demons of Segfault—that adventure is just beginning.
...
Congratulations, brave traveler. You have earned your rest.
